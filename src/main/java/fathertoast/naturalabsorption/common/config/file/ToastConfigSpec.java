...
public class ToastConfigSpec {
    ...
    public ToastConfigSpec( File dir, String fileName ) {
        ...
        if( !dir.exists() && !dir.mkdirs() ) {
            NaturalAbsorption.LOG.error( "Failed to make config folder! Things will likely explode. " +
    ...
    public void initialize() {
        NaturalAbsorption.LOG.info( "First-time loading config file {}", CONFIG_FILE.getFile() );
        firstLoad = true;
        CONFIG_FILE.load();
    }
    ...
    private static class Header extends Format {
        ...
        @Override
        public void write( ToastTomlWriter writer, CharacterOutput output ) {
            writer.writeComment( NaturalAbsorption.MOD_ID + ":" + PARENT.NAME + ToastConfigFormat.FILE_EXT, output );
