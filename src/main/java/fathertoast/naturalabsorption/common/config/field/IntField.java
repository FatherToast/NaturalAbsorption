...
public class IntField extends AbstractConfigField {
    ...
    public void load( @Nullable Object raw ) {
        ...
        if( raw instanceof Number ) {
            ...
            if( rawValue < valueMin ) {
                NaturalAbsorption.LOG.warn( "Value for {} \"{}\" is below the minimum ({})! Clamping value. Invalid value: {}",
                        getClass(), getKey(), valueMin, raw );
                newValue = valueMin;
            }
            else if( rawValue > valueMax ) {
                NaturalAbsorption.LOG.warn( "Value for {} \"{}\" is above the maximum ({})! Clamping value. Invalid value: {}",
                        getClass(), getKey(), valueMax, raw );
                newValue = valueMax;
            }
            else {
                if( (double) rawValue != ((Number) raw).doubleValue() ) {
                    NaturalAbsorption.LOG.warn( "Value for {} \"{}\" is not an integer! Truncating value. Invalid value: {}",
                            getClass(), getKey(), raw );
                }
                newValue = rawValue;
            }
            ...
            NaturalAbsorption.LOG.info( "Unboxing string value for {} \"{}\" to a different primitive.",
                    getClass(), getKey() );
            ...
            if( raw != null ) {
                NaturalAbsorption.LOG.warn( "Invalid value for {} \"{}\"! Falling back to default. Invalid value: {}",
                        getClass(), getKey(), raw );
            }
    ...
    public static class RandomRange {
        ...
        public int next( Random random ) {
            ...
            if( delta < 0 ) {
                NaturalAbsorption.LOG.warn( "Value for range \"({},{})\" is invalid ({} > {})! Ignoring maximum value.",
